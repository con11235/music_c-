...
public:
	...
	*/
	...
	void operator = (DoublyLinkedList item);
	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			if (item<itor.m_pCurPointer->data) // 맞는 자리를 찾는다.
			{
				...
				return 0; // 0을 반환.
			else if (!itor.NextNotNull()) {
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
				pItem->prev = itor.m_pCurPointer;
				pItem->next = itor.m_pCurPointer->next;
				itor.m_pCurPointer->next->prev = pItem;
...
template <typename T>
void DoublyLinkedList<T>::operator = (DoublyLinkedList item) {
	this->MakeEmpty();
	this->m_pFirst = item.m_pFirst;
	this->m_pLast = item.m_pLast;
	this->m_nLength = item.m_nLength;
}
